...
def test_basic_setup():
    ...
    return (
        config.ANTHROPIC_API_KEY is not None and config.ANTHROPIC_API_KEY.strip() != ""
    )
...
def test_vector_store_status(rag_system):
    ...
    try:
        ...
        if course_count == 0:
            print(
                "[WARNING] No courses found in vector store - this might be the issue!"
            )
...
def test_simple_query(rag_system):
    ...
    try:
        ...
        if "No relevant content found" in result:
            print(
                "[WARNING] Tool executed but found no content - vector store might be empty"
            )
        else:
...
def main():
    ...
    if not has_api_key:
        print(
            "[ERROR] Missing or invalid API key - create .env file with ANTHROPIC_API_KEY"
        )
    if not has_data:
